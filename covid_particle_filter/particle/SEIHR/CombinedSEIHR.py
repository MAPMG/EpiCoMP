"""
The objects in this sub-module allow the user to combine the SEIHR particle
...
The SEIHR_combined_particle object in this sub-module allows the user
...
The SEIHR_forecast_manager is a meta-object that manages multiple
SEIHR_combined_particle objects, allowing the user to create many parallel
...
class SEIHR_combined_particle():
    """An object to combine the SEIHR particle object with the HCompartment."""
    ...
    def __init__(
        ...
        """Initialize an SEIHR_combined_particle.
    ...
    def _set_current(self, admits, discharges, open_admit_dts):
        ...
        cur_r = np.sum(np.array(self.hist_particle.cur_vals)[[3,4]]) - cur_h
        ...
        self.base_fut_particle = self.hist_particle.__class__(
            ...
            init_vals = np.array(
                [cur_s,cur_e,cur_i,cur_h,cur_r]
            ),
            param_generator = self.hist_particle.param_generator,
            t_init = t,
    ...
    def full_forecast(self):
        ...
        This forecast call alters the seihr_forecast call.  It adjusts the H compartment to be instantaneous
        ...
        seihr_ = self.seihr_forecast
        disc = self.h_compartment.get_history(self.fut_particle._t_history)
        
        h = seihr_[:, 3] - disc
        
        return np.hstack([seihr_[:, [0,1,2]], h.reshape(-1,1), seihr_[:,4].reshape(-1,1), disc.reshape(-1,1)])
    
    
class SEIHR_forecast_manager():
    ...
    def __init__(
        ...
        """Initialize an SEIHR_forecast_manager object.
        
        This method initializes a forecast manager; in addition, it also creates a collection of 
        independent SEIHR_combined_particle objects, one for each particle in the given particle_ls
        ...
        Args:
            particle_ls (list of SEIHR_Particle objects): the list of particles to use in forecasting
            h_compartment (HCompartment): An HCompartment object fit using this package
            admits (int): The total number of admissions to-date
            discharges (int): The total number of discharges to-date
            open_admit_dts (list): A list of the dates of admissions without corresponding discharges
